...
#Calculer le potentiel avec la méthode de sûr-relaxation
différence = 0.001
omega = 0.9
    ...
    for x in range(121):
        for y in range(61):
            if detecteur[y, x] != 0 and detecteur[y, x] != 50 and detecteur[y, x] != -300 :
                if y == 30:
                    detecteur[y, x] = detecteur[y, x] + (1 + omega)*(((detecteur[y, x+1] + detecteur[y, x-1])/2)-detecteur[y, x])
                else :
                    detecteur[y, x] = detecteur[y, x] + (1+omega)*(((detecteur[y+1, x] + detecteur[y-1, x] + detecteur[y, x+1] + detecteur[y, x-1])/4 + (detecteur[y+1, x] - detecteur[y-1, x])/(8*(30-y)))-detecteur[y, x])
